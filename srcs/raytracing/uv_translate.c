...
t_vector	get_color_plane(t_plane *plane, t_raytrace_info *raytrace)
{
	...
	raytrace->u_val = 1.0 - dot_v(p_local, raytrace->tangent);
	raytrace->v_val = dot_v(p_local, raytrace->bitangent);
...
t_vector	get_color_cylinder(t_cylinder *cylinder, t_raytrace_info *raytrace)
{
	...
	raytrace->v_val = 8 * dot_v(p_local, cylinder->orientation)
		/ cylinder->height;
...
t_vector	get_color_cone(t_cone *cone, t_raytrace_info *raytrace)
{
	...
	height_proj = dot_v(subtract_v(raytrace->inter, cone->position),
			cone->orientation);
	angle = atan2(dot_v(raytrace->inter, raytrace->bitangent),
			dot_v(raytrace->inter, raytrace->tangent));
