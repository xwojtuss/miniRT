...
/*   Updated: 2024/12/31 20:43:58 by wkornato         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_t_cone(t_cone *cone, t_ray ray, double *t1, double *t2)
{
	...
	origin_to_center = subtract_v(ray.origin, add_v(cone->position,
				multiply_v(cone->orientation, cone->height)));
	a = dot_product(ray.direction, ray.direction) - (1 + tan_theta_squared)
		* pow(dot_product(ray.direction, cone->orientation), 2);
	b = 2 * (dot_product(ray.direction, origin_to_center) - (1
				+ tan_theta_squared) * dot_product(ray.direction,
				cone->orientation) * dot_product(origin_to_center,
				cone->orientation));
	discriminant = b * b - 4 * a * dot_product(origin_to_center,
			origin_to_center) - (1 + tan_theta_squared)
		* pow(dot_product(origin_to_center, cone->orientation), 2);
	if (discriminant < 0)
		return ;
	retrieve_t(a, b, discriminant, (double *[2]){t1, t2});
}