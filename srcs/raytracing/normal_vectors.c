...
/*   Updated: 2024/12/31 21:23:43 by wkornato         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (dot_v(subtract_v(ray.origin, plane->position),
			plane->orientation) > 0)
...
t_vector	get_nv_cylinder(t_vector intersect, t_cylinder *cylinder, t_ray ray)
{
	...
	return (normalize_vector(scale_v(normal, -1)));
}
...
t_vector	get_nv_cone(t_vector intersect, t_cone *cone, t_ray ray)
{
	...
	return (scale_v(normalize_vector(radial), -1));
}